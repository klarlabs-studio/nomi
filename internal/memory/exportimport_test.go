...
func TestExportImport_RoundTrip(t *testing.T) {
	...
	for _, body := range []string{"alpha", "beta", "gamma", "delta"} {
		if err := src.Store(ctx, scope, &mnemos.Entry{Content: body}); err != nil {
	...
	got, err := dst.Retrieve(ctx, scope, mnemos.Query{Limit: 100})
	if err != nil {
		t.Fatal(err)
	}
	if len(got) != 4 {
...
func TestImport_RejectsBadVersion(t *testing.T) {
	...
	body := `{"format":"mnemos.export","version":99,"scope":{"owner_id":"local","kind":"workspace","key":"default"}}` + "\n"
	_, err := Import(context.Background(), c, strings.NewReader(body))
	if err == nil {
...
func TestImport_RejectsInvalidScopeInHeader(t *testing.T) {
	...
	body := `{"format":"mnemos.export","version":1,"scope":{"kind":"workspace","key":"default"}}` + "\n"
	_, err := Import(context.Background(), c, strings.NewReader(body))
	if err == nil {
...
func TestExport_RejectsInvalidScope(t *testing.T) {
	...
	var buf bytes.Buffer
	_, err := Export(context.Background(), c, mnemos.Scope{}, &buf)
	if err == nil {
		t.Fatal("expected error on invalid scope")
	}
}
