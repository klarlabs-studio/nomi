...
func (t *postMessageTool) Name() string       { return "imessage.post_message" }
func (t *postMessageTool) Capability() string { return "imessage.post" }
...
func (t *postMessageTool) Execute(ctx context.Context, input map[string]interface{}) (map[string]interface{}, error) {
	...
	if connectionID == "" {
		return nil, fmt.Errorf("imessage.post_message: connection_id is required")
	}
	chatGUID, _ := input["chat_guid"].(string)
	if chatGUID == "" {
		return nil, fmt.Errorf("imessage.post_message: chat_guid is required")
	}
	text, _ := input["text"].(string)
	if text == "" {
		return nil, fmt.Errorf("imessage.post_message: text is required")
	}
	...
	if assistantID != "" && t.plugin.bindings != nil {
		...
		if err != nil {
			return nil, fmt.Errorf("imessage.post_message: binding check failed: %w", err)
		}
		if !ok {
	...
	t.plugin.mu.RUnlock()
	if !ok {
		return nil, fmt.Errorf("imessage.post_message: connection %s is not running", connectionID)
	}
	if err := cli.sendText(ctx, chatGUID, text); err != nil {
		return nil, fmt.Errorf("imessage.post_message: %w", err)
	}
	return map[string]interface{}{"chat_guid": chatGUID}, nil
}
