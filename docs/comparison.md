...
| Connector breadth (shipped) | Telegram, Slack, Discord, WhatsApp, Matrix, Teams, Signal, Email, Gmail, GitHub, Obsidian, Calendar + generic MCP | 20+ messaging apps + email | 13+ (WhatsApp, Telegram, Slack, Teams, Gmail, iMessage, Matrix, GitHub, Linear, …) | Telegram-primary + others | n/a |
...
- **Connector breadth today.** OpenClaw still leads on raw messaging
  ...
  Teams, Signal, Email, Gmail, GitHub, Obsidian, Calendar, plus a
  generic MCP plugin so any MCP server (filesystem, postgres, github,
  browser, …) is a capability-gated tool without writing a Nomi plugin.
...
- **Connectors today.** NanoClaw ships ~13 messaging connectors out of
  ...
  Teams, Signal, Email, Gmail, GitHub, Obsidian, Calendar, and generic
  MCP.
...
- **OpenClaw still leads on raw messaging-app count.** Nomi ships
  Telegram, Slack, Discord, WhatsApp, Matrix, Teams, Signal, Email,
  plus Gmail / GitHub / Obsidian / Calendar plugins and generic MCP.
  The long tail of iMessage / Beeper / 20+ chat apps is still
  OpenClaw's (Beeper can use Nomi's Matrix plugin via bridges; Signal
  needs a local signal-cli-rest-api sidecar).
